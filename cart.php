<?php
    ...
    $connect = mysqli_connect("localhost", "root", "", "shopping") or die("Please check your server");
    $message = "";
    $quantity = "";
    $action = "";
    $query = "";
    if ($isset($_POST['quantity'])) {
        $qunatity = trim($_POST['quantity']);
    }
    if ($quantity == '')
    {
        $quantity = 1;
    ...
    if ($quantity < 0) {
        ...
        switch ($action) {
            case "add": $query = "select * from cart where cart_sess = '$sess' and cart_itemcode like '$itemcode'";
